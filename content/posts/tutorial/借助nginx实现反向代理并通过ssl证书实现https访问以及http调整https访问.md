---
title: "借助nginx实现反向代理，并通过SSL证书实现https访问以及http调整https访问。"
...
tags: # 标签
- 文章
description: "借助nginx实现反向代理，并通过SSL证书实现https访问以及http调整https访问。"
weight:
slug: ""
draft: false # 是否为草稿
...
*
  ...
  * [申请SSL证书](#SSL_45)
  * [首先安装nginx](#nginx_53)
  * [上传.crt和.key证书](#crtkey_57)
  * [创建配置文件](#_68)
  * [验证配置文件并重启nginx服务](#nginx_106)
* [成功](#_120)
* [若有帮助，希望可以点赞收藏😁。](#_123)
...
能找到的大多数资料都是基于 [nginx](https://zh.wikipedia.org/wiki/Nginx) 实现的，那么什么是 nginx 呢？
> Nginx（发音同"engine X"）是异步框架的网页服务器，也可以用作反向代理、负载平衡器和HTTP缓存。 *------摘自维基百科*

对于不熟悉web的人，或者小白可以将其看作一网页服务器就好，我们这里需要用到的主要有**反向代理**。
...
在宿舍这可看作一个小的LAN，那位可以用手机上网的同学则充当了**正向代理** 的作用，即我们一般意义上的**代理** 。  
反向代理：
...
以上内容来自：[php中文网](https://www.php.cn/nginx/457309.html)  
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20210123145524725.png#pic_center)
